...
游戏头像的流程是：拍照 → 照片生成并落盘 → 上传给服务器 → 服务器校验并认证。本工具在"拍照完成之后、交给服务器之前"抢先替换照片内容：

- D3D11 钩子（MinHook 拦截 `ID3D11DeviceContext`）：在照片离屏纹理被复制/映射读取的时机，把目标尺寸的纹理内容替换为自定义图片；替换完成后，游戏后续的编码与上传流程保持不变，交出去的就是自定义图
- 照片文件兜底（`tools/watch_photos.py`）：照片落盘后把本地文件也替换为自定义图（原文件自动备份为 `*.watcherbak`），覆盖上传时直接读取文件的路径
- 服务器收到的请求与正常客户端完全一致，只是内容源头已被替换，校验认证通过后盖章的即为自定义图片

> 效果与在网络层篡改上传数据等价，但本工具不修改网络请求、不安装证书、不碰系统代理，仅在本地渲染与文件环节替换照片内容。
